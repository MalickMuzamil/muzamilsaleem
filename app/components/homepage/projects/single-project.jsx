...
const SingleProject = ({ project }) => {
  const { name, description, code, demo, image } = project;
    ...
    >
      ...
      <div className='flex flex-col items-center justify-between w-full h-full'>
        ...
        <div className="p-6 flex justify-center">
          { /* Accept either a string public path (e.g. '/png/my.png') or an object with .src */ }
          {(() => {
            const imageSrc = typeof image === 'string' ? image : (image?.src || '/png/placeholder.png');
            return (
              <Image
                src={imageSrc}
                alt={name}
                width={1080}
                height={720}
                // Make image responsive: full width up to max, keep aspect ratio
                className="w-full max-w-[900px] h-auto rounded-lg transition-opacity duration-[0.7s] delay-[0.3s]"
              />
            );
          })()}
        </div>
